    ...
    public void actionPerformed(ActionEvent e)
    {
        if(e.getSource()==b1)
        {
            ...
            try
            {
                ...
                ResultSet myRs = myStmt.executeQuery("select max(id) from appointmentbookingsystem."+n);
                if(myRs.next())
                {
                    max=myRs.getInt(1);
                }
                ++max;
                String sq1 = "insert into "+n+" (id, full_name, age, contact) values ('"+max+"', '"+s1+"', '"+age+"', '"+pno+"')";
